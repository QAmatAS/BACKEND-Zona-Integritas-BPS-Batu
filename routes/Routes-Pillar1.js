...
const PillarSatu = require('../models/Model-Pillar1');
...
router.get('/', async (req, res) => {
    try {
        const samples = await PillarSatu.find({});
...
router.get('/rincian-by-id/:rencanaAksiId/:rincianKegiatanId', async (req, res) => {
    ...
    try {
        const rencanaAksi = await PillarSatu.findOne(
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const existingRencanaAksi = await PillarSatu.findOne({ id: newRencanaAksi.id });
        ...
        const createdRencanaAksi = await PillarSatu.create(newRencanaAksi);
...
router.post('/add-rincian/:id', async (req, res) => {
    ...
    try {
        const updatedRencanaAksi = await PillarSatu.findOneAndUpdate(
            { id: parseInt(id) },
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        const result = await PillarSatu.deleteOne({ id: parseInt(id) });
