...
const PillarEmpat = require('../models/Model-Pillar4');
...
router.get('/', async (req, res) => {
    try {
        const samples = await PillarEmpat.find({});
...
router.get('/rincian-by-id/:rencanaAksiId/:rincianKegiatanId', async (req, res) => {
    ...
    try {
        const rencanaAksi = await PillarEmpat.findOne(
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const existingRencanaAksi = await PillarEmpat.findOne({ id: newRencanaAksi.id });
        ...
        const createdRencanaAksi = await PillarEmpat.create(newRencanaAksi);
...
router.post('/add-rincian/:id', async (req, res) => {
    ...
    try {
        const updatedRencanaAksi = await PillarEmpat.findOneAndUpdate(
            { id: parseInt(id) },
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        const result = await PillarEmpat.deleteOne({ id: parseInt(id) });
