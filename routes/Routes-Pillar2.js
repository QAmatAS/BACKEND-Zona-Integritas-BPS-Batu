...
const PillarDua = require('../models/Model-Pillar2');
...
router.get('/', async (req, res) => {
    try {
        const samples = await PillarDua.find({});
...
router.get('/rincian-by-id/:rencanaAksiId/:rincianKegiatanId', async (req, res) => {
    ...
    try {
        const rencanaAksi = await PillarDua.findOne(
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const existingRencanaAksi = await PillarDua.findOne({ id: newRencanaAksi.id });
        ...
        const createdRencanaAksi = await PillarDua.create(newRencanaAksi);
...
router.post('/add-rincian/:id', async (req, res) => {
    ...
    try {
        const updatedRencanaAksi = await PillarDua.findOneAndUpdate(
            { id: parseInt(id) },
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        const result = await PillarDua.deleteOne({ id: parseInt(id) });
