...
router.get('/', async (req, res) => {
    try {
        const samples = await PillarEnam.find({});
...
router.get('/rincian-by-id/:rencanaAksiId/:rincianKegiatanId', async (req, res) => {
    ...
    try {
        const rencanaAksi = await PillarEnam.findOne(
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const existingRencanaAksi = await PillarEnam.findOne({ id: newRencanaAksi.id });
        ...
        const createdRencanaAksi = await PillarEnam.create(newRencanaAksi);
...
router.post('/add-rincian/:id', async (req, res) => {
    ...
    try {
        const updatedRencanaAksi = await PillarEnam.findOneAndUpdate(
            { id: parseInt(id) },
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        const result = await PillarEnam.deleteOne({ id: parseInt(id) });
